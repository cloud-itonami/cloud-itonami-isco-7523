(ns woodworking.actor
  "WoodworkingMachineOperationsActor — the ISCO-08 7523 community woodworking actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one woodworking operation request
  ...
  The unconditional invariant: the WoodworkingMachineOperationsAdvisor can never
  directly commit a record the WoodworkingMachineOperationsGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [woodworking.advisor :as advisor]
            [woodworking.governor :as governor]
            [woodworking.store :as store]))

(defn build-graph
  "Build a compiled WoodworkingMachineOperationsActor graph. `store` implements
  `woodworking.store/Store`. `advisor` implements
  `woodworking.advisor/Advisor` (defaults to `mock-advisor`).
